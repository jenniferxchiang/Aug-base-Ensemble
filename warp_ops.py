...
class ResizeTransformTime(nn.Module):
    ...
    def forward(self, x):
        factor = self.sf
        if factor < 1:
            x = F.interpolate(x, align_corners=False, scale_factor=factor,
                              mode=self.mode, recompute_scale_factor=False)
            x = factor * x
        elif factor > 1:
            x = factor * x
            x = F.interpolate(x, align_corners=False, scale_factor=factor,
...
class RandWarpAug(nn.Module):
    def __init__(self, inshape, int_steps=7, int_downsize=4, smooth_size=51):
        ...
        self.resize = ResizeTransformTime(
            1/int_downsize, ndims) if resize else None
        self.fullsize = ResizeTransformTime(
        ...
        filt = gaussian_filter1d(
            smooth_kernel, smooth_centre).astype(np.float32)
    ...
    def forward(self, source, flow_mag):
        ...
        pos_flow = F.conv1d(pos_flow, self.smooth_kernel.view(
            1, 1, self.smooth_size).to(x.device), padding=self.smooth_pad, stride=1)
